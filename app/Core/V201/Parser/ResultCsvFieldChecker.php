...
class ResultCsvFieldChecker
{
    ...
    /**
     * ResultCsvFieldChecker constructor.
     */
    ...
    /**
     * checks if current row is result
     * @return string
     */
    protected function isResult()
    {
        return getVal($this->csvRow, ['type']);
    }


    /**
     * add rules and messages for result type
     */
    public function checkType()
    {
        if ($this->isResult()) {
            $this->rules['type']              = 'required|code_list:Activity,ResultType';
            $this->messages['type.required']  = 'Result Type is required.';
            $this->messages['type.code_list'] = 'Result Type is not valid.';
        }
    }
     ...
     * add rules and messages for result type
     */
    public function checkAggregationStatus()
    {
        if ($this->isResult()) {
            $this->rules['aggregation_status']              = 'in:0,1';
            $this->messages['aggregation_status.code_list'] = 'Aggregation Status is not valid.';
        }

        dd($this->csvRow, $this->rules);
    }

    /**
     * add rules and messages for result title
     */
    public function checkTitle()
    {
        if ($this->isResult()) {
            $this->rules['title']             = 'required';
            $this->messages['title.required'] = 'Activity Title is required.';
        }
    }
