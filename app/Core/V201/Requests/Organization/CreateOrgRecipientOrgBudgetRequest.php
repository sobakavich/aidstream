...
class CreateOrgRecipientOrgBudgetRequest extends OrganizationBaseRequest
{
    ...
    public function rules()
    {
        ...
        foreach ($this->get('recipient_organization_budget') as $recipientOrganizationBudgetIndex => $recipientOrganizationBudget) {
            ...
            $narrativeKeys                   = array_keys($recipientOrganizationBudget['recipient_organization'][0]['narrative']);
            $narrativeField                  = sprintf('%s.recipient_organization.0.narrative.%s.narrative', $recipientOrganizationBudgetForm, $narrativeKeys[0]);
    ...
    public function messages()
    {
        ...
        foreach ($this->get('recipient_organization_budget') as $recipientOrganizationBudgetIndex => $recipientOrganizationBudget) {
            $recipientOrganizationBudgetForm = sprintf('recipient_organization_budget.%s', $recipientOrganizationBudgetIndex);
            $narrativeKeys                   = array_keys($recipientOrganizationBudget['recipient_organization'][0]['narrative']);
            $narrativeField                  = sprintf('%s.recipient_organization.0.narrative.%s.narrative.required_without', $recipientOrganizationBudgetForm, $narrativeKeys[0]);
