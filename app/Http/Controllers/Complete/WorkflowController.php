...
class WorkflowController extends Controller
{
    ...
    public function publish($id, Request $request)
    {
        ...
        if (null === $this->workFlowManager->publish($activity, $request->all())) {
            return redirect()->back()->withResponse(['type' => 'warning', 'code' => ['message', ['message' => 'Something does not seem to be right.']]]);
        }

        if ($message = $this->workFlowManager->publish($activity, $request->all())) {
            return redirect()->back()->withResponse(['type' => 'warning', 'code' => ['message', ['message' => 'Could not publish to registry. (' . $message . ')']]]);
        }
