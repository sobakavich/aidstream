...
class TransactionDataMappingTest extends AidStreamTestCase
{
    ...
    public function setUp()
    {
        ...
        $this->xmlServiceProvider = new XmlServiceProvider($this->service);
        $xmlData                  = file_get_contents('tests/app/xmlImporter/xml/V202.xml');
        $this->data               = $this->xmlServiceProvider->load($xmlData);
        $this->template           = $template = json_decode(file_get_contents(app_path('/Services/XmlImporter/Templates/V202.json')), true);
    }
