...
<ol>
    @foreach($users as $user)
        ...
        if ($user->role_id === 1) {
            ...
            $userType = 'Secondary Contact at Organisation';
        } else {
            $permissions = [];
            foreach ($user->user_permission as $permissionKey => $permission) {
                $permissions[] = sprintf('%s:%s', $permissionKey, $permission);
            }
            $userType = $code->getOrganizationCodeName('Permissions', implode(',', $permissions), false);
        }
        ?>
        <li>{{ $userType }}: {{ $user->first_name }} {{ $user->last_name }} (Username: {{ $user->username }}) - {{ $user->email }}</li>
    @endforeach
</ol>
<p>
